...
/**
 * The AST node for a unary addition (+) expression.
 */

class JUnaryAddOp extends JUnaryExpression {

    /**
     * Construct an AST node for a unary add expression given its line number,
     * and the operand.
     * 
     * @param line
     *            line in which the unary add expression occurs in the source
     *            file.
     * @param arg
     *            the operand.
     */

    public JUnaryAddOp(int line, JExpression arg) {
        super(line, "+", arg);
    }

    /**
     * Analyzing the unary addition operation involves analyzing its operand, checking
     * its type and determining the result type.
     * 
     * @param context
     *            context in which names are resolved.
    ...
    /**
     * Generating code for the unary add operation involves generating code for
     * the operand, and then the unary add instruction.
     * 
     * @param output
     *            the code emitter (basically an abstraction for producing the
     *            .class file).
